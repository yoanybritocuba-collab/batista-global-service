...
import { Search, X, TrendingUp, Clock, Mic, Loader } from 'lucide-react';
...
const SearchBar = ({ variant = 'default', onClose }) => {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      if (query.length >= 2) {
        setIsLoading(true);
        const results = getSuggestions ? getSuggestions(query) : [];
  ...
  const handleSearch = (e) => {
    e.preventDefault();
    if (query.trim()) {
      if (addRecentSearch) addRecentSearch(query);
  ...
  const handleSuggestionClick = (suggestion) => {
    if (addRecentSearch) addRecentSearch(suggestion.name);
  ...
  return (
    <div className="relative w-full" ref={suggestionsRef}>
      ...
      {/* Sugerencias */}
      {showSuggestions && (suggestions.length > 0 || (recentSearches && recentSearches.length > 0) || (popularSearches && popularSearches.length > 0)) && (
