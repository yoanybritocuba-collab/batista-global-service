      ...
      <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        ...
        <div className="w-full px-6 lg:px-8">
          <div className="flex items-center justify-between">
            {/* LOGO Y ADMIN - MÁS GRANDES */}
            <div className="flex items-center gap-4">
              {/* Logo - MÁS GRANDE (ocupa todo el alto) */}
              <button 
                onClick={goToHome}
                className="flex items-center gap-3 h-14 lg:h-16"
                aria-label="Ir al inicio"
              >
                <img src="/images/logo-icon.png" alt="Batista" className="h-full w-auto" />
                <img src="/images/logo-text.png" alt="Batista Global Service" className="h-10 lg:h-12 w-auto hidden sm:block" />
              </button>

              {/* Icono de ADMIN */}
              <button 
                onClick={goToAdmin}
                className="p-3 hover:bg-amber-600/20 rounded-full transition-colors relative group"
                aria-label="Panel de Administración"
                title="Acceso Administrativo"
              >
                <Shield className="h-6 w-6 text-amber-400 group-hover:text-amber-300" />
            ...
            <div className="flex items-center gap-3">
              ...
              >
                {language === 'es' ? (
                  <img src="https://flagcdn.com/w40/es.png" alt="Español" className="w-7 h-7 rounded-full border border-white/30" />
                ) : (
                  <img src="https://flagcdn.com/w40/us.png" alt="English" className="w-7 h-7 rounded-full border border-white/30" />
                )}
              </button>

              {/* User Menu (Cliente) */}
              <Link to={isAuthenticated ? "/cliente/perfil" : "/cliente/login"} className="p-3 hover:bg-white/10 rounded-full transition-colors relative">
              ...
              {/* Cart */}
              <Link to="/cart" className="p-3 hover:bg-white/10 rounded-full transition-colors relative">
          ...
          {isMenuOpen && (
            <div className="lg:hidden py-4 border-t border-white/10 mt-4">
              ...
              <nav className="flex flex-col space-y-2">
                ...
                <button onClick={goBack} className="flex items-center gap-3 px-4 py-4 hover:bg-white/10 rounded-lg text-white/80 hover:text-amber-400 text-lg">
                  <ArrowLeft className="h-6 w-6" /> <span>{t('atras')}</span>
                </button>
