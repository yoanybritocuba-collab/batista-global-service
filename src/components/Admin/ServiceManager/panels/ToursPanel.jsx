...
const ToursPanel = ({ data, onChange }) => {
  ...
  const localData = {
    // RANGOS DE PRECIOS GENERALES - ¡ESTO ES LO QUE FALTA!
    rangosPrecios: {
      minimo: data?.rangosPrecios?.minimo || 79,
      maximo: data?.rangosPrecios?.maximo || 599,
      promedio: data?.rangosPrecios?.promedio || 250
    },
    // Destinos
    destinos: data?.destinos || [],
    // Paquetes generales (si no hay destinos)
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          ...
          <div className="space-y-2">
            {localData.incluye.map((item, idx) => (
              <div key={idx} className="flex gap-2">
                <input
                  type="text"
                  value={item}
                  onChange={(e) => updateListItem('incluye', idx, e.target.value)}
                  className="flex-1 px-3 py-2 border rounded-lg"
                />
                <button onClick={() => removeFromList('incluye', idx)} className="text-red-500">
                  <X className="h-5 w-5" />
                </button>
              </div>
            ))}
            <button
              onClick={() => addToList('incluye', '')}
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          ...
          <div className="space-y-2">
            {localData.noIncluye.map((item, idx) => (
              <div key={idx} className="flex gap-2">
                <input
                  type="text"
                  value={item}
                  onChange={(e) => updateListItem('noIncluye', idx, e.target.value)}
                  className="flex-1 px-3 py-2 border rounded-lg"
                />
                <button onClick={() => removeFromList('noIncluye', idx)} className="text-red-500">
                  <X className="h-5 w-5" />
                </button>
              </div>
            ))}
            <button
              onClick={() => addToList('noIncluye', '')}
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        ...
        <div className="space-y-2">
          {localData.recomendaciones.map((item, idx) => (
            <div key={idx} className="flex gap-2">
              <input
                type="text"
                value={item}
                onChange={(e) => updateListItem('recomendaciones', idx, e.target.value)}
                className="flex-1 px-3 py-2 border rounded-lg"
              />
              <button onClick={() => removeFromList('recomendaciones', idx)} className="text-red-500">
                <X className="h-5 w-5" />
              </button>
            </div>
          ))}
          <button
            onClick={() => addToList('recomendaciones', '')}
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        ...
        <div className="space-y-4">
          {localData.ofertas.map((oferta, idx) => (
            <div key={idx} className="bg-purple-50 rounded-lg p-4">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-2">
                <input
                  ...
                  value={oferta.nombre || ''}
                  onChange={(e) => {
                    const newOfertas = [...localData.ofertas];
                    newOfertas[idx].nombre = e.target.value;
                    onChange({ ...localData, ofertas: newOfertas });
                  }}
                  placeholder="Nombre"
                  className="px-3 py-2 border rounded-lg"
                />
                <input
                  ...
                  onChange={(e) => {
                    const newOfertas = [...localData.ofertas];
                    newOfertas[idx].descuento = e.target.value;
                    onChange({ ...localData, ofertas: newOfertas });
                  }}
                ...
                <input
                  type="text"
                  value={oferta.condiciones || ''}
                  onChange={(e) => {
                    const newOfertas = [...localData.ofertas];
                    newOfertas[idx].condiciones = e.target.value;
                    onChange({ ...localData, ofertas: newOfertas });
                  }}
              ...
              <button onClick={() => removeFromList('ofertas', idx)} className="text-red-500 text-sm">
                Eliminar oferta
              </button>
            </div>
          ))}
          <button
            onClick={() => {
              const newOfertas = [...localData.ofertas, { nombre: '', descuento: '', condiciones: '' }];
              onChange({ ...localData, ofertas: newOfertas });
            }}
